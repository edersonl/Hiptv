...
namespace IptvStarterApp.UI
{
    ...
    public class HomeActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle? savedInstanceState)
        {
            ...
            if (_playlistUrl is not null) _playlistUrl.Text = _channelStore.LastPlaylistUrl;
            if (_connectButton is not null) _connectButton.Click += async (_, _) => await ConnectPlaylistAsync();

            FindViewById<Button>(Resource.Id.openLiveButton)!.Click += (_, _) => OpenChannels();
            FindViewById<Button>(Resource.Id.openFavoritesButton)!.Click += (_, _) => OpenFavorites();
            FindViewById<Button>(Resource.Id.openSearchButton)!.Click += (_, _) =>
        ...
        private async Task ConnectPlaylistAsync()
        {
            ...
            {
                Android.Util.Log.Warn("Hiptv/Home", exception.GetType().Name);
                SetStatus("Não foi possível carregar a playlist. Verifique a URL e a conexão.");
            }
            finally
            {
        ...
        {
            if (_channelStore is null || _favoritesService is null) return;
            RenderChannelButtons(_recentContainer, _channelStore.GetRecentChannels(), "Nenhum canal recente");
            RenderChannelButtons(_favoritesContainer, _favoritesService.GetFavorites(), "Seus favoritos aparecerão aqui");

            if (_categoriesContainer is null) return;
            _categoriesContainer.RemoveAllViews();
            var groups = _channelStore.GetChannels().Select(channel => channel.Group)
                .Where(group => !string.IsNullOrWhiteSpace(group))
                .Distinct(StringComparer.OrdinalIgnoreCase).OrderBy(group => group).Take(12).ToArray();
            if (groups.Length == 0)
            {
                _categoriesContainer.AddView(CreateEmptyText("Conecte uma playlist para ver categorias"));
                return;
            }

            foreach (var group in groups)
            {
                var button = CreateChannelButton(group);
                button.Click += (_, _) => OpenChannels(group);
                _categoriesContainer.AddView(button);
            }
        }

        private void RenderChannelButtons(LinearLayout? container, IEnumerable<ChannelItem> channels, string emptyText)
        {
        ...
        private Button CreateChannelButton(string text)
        {
            ...
            button.LayoutParameters = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WrapContent, LinearLayout.LayoutParams.WrapContent) { RightMargin = 12 };
            return button;
        }

        private TextView CreateEmptyText(string text)
        {
            var view = new TextView(this) { Text = text, TextSize = 15 };
            view.SetTextColor(Android.Graphics.Color.ParseColor("#9CA3AF"));
            return view;
        }
        ...
        private void OpenChannels(string? group = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(group)) intent.PutExtra("selected_group", group);
